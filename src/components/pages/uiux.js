...
function Research(props) {
    ...
    return (
        ...
        {content.map((item, key) => ( // can't use forEach, it can't display properly
            <div className={item.inline === true ? 'd-flex': 'd-block'} key={key}>
                <div>
                    <h4>{item.title}</h4>
                    <p>{item.paragraph}</p>
                </div>
...
function PPAnalysis(props) {
    const { content } = props;
    const list = content.map(item => (<li>{item}</li>))
...
function Mockup(props) {
    ...
    return (
        <>
           {content.map((item, key) => ( // can't use forEach, it can't display properly
                <div className={item.inline === true ? 'd-flex mockupSec': 'd-block mockupSec'} key={key}>
                    <div>
                        <h4>{item.title}</h4>
